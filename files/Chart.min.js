...
(function () {
    ...
    extend(Chart.Type.prototype, {
        ...
        render: function (reflow) {
            ...
            if (this.options.tooltipAnimation) {
                if (this.tooltipTimeout)
                    clearTimeout(this.tooltipTimeout)
                var self = this, isAllNull = true;
                self.points = [];
                ...
                var pointsIndex = 0;
                self.loopPoints = function () {
                    if(isAllNull)
                        return;
                    try {
                        if (self.points[pointsIndex].value != null) {
                            self.showTooltip([self.points[pointsIndex]]);
                            self.tooltipTimeout = setTimeout(self.loopPoints, 4000)
                            if (pointsIndex < self.points.length / 2 - 1)
                                pointsIndex++;
                            else
                                pointsIndex = 0;
                        } else {
                ...
                this.tooltipTimeout = setTimeout(self.loopPoints, 4000)
        ...
        showTooltip: function (ChartElements, forceRedraw) {
            ...
            if (ChartElements.length > 0) {
                ...
                if (this.datasets && this.datasets.length > 1 && !this.options.isUseToolTip) {
                    var dataArray,
                        dataIndex;
        ...
        draw: function () {
            ...
            if (this.text.indexOf(',') > 0) {
                ...
                for (var i = 0; i < this.text.split(',').length; i++) {
                    if (ctx.measureText(this.text.split(',')[i]).width > maxlength) {
                        maxlength = ctx.measureText(this.text.split(',')[i].substr(0, 12)).width;
                    }
                }
                tooltipWidth = maxlength + 2 * this.xPadding;
                tooltipRectHeight = (this.fontSize + 2 * this.yPadding) * this.text.split(',').length;
                tooltipHeight = tooltipRectHeight + this.caretHeight + caretPadding;
            }

            if (this.x + 7 + tooltipWidth / 2 > this.chart.width) {
                this.xAlign = "left";
            } else if (this.x - 7 - tooltipWidth / 2 < 0) {
                this.xAlign = "right";
            }
            ...
            if (this.custom) {
                ...
                if (this.text.indexOf(',') > 0) {
                    for (var j = 0; j < this.text.split(',').length; j++) {
                        var y = ((tooltipRectHeight / (this.text.split(',').length + 1)) * (j + 1));
                        var _text = this.text.split(',')[j];
                        if (_text.length > 12)
                            _text = _text.substr(0, 10) + '...'
                        ctx.fillText(_text, tooltipX + 7, tooltipY + y);
                    }
                } else
                    ctx.fillText(this.text, tooltipX + 7, tooltipY + tooltipRectHeight / 2);
            }
        }
    });
...
(function () {
    ...
    function indexOfMin(arr) {
        ...
        return arr[minIndex] < 15 ? minIndex : null;
    }
        ...
        initialize: function (data) {
            ...
            if (this.options.showTooltips) {
                ...
                helpers.bindEvents(this, this.options.tooltipEvents, function (evt) {
                    ...
                    if (self.options.isUseToolTip) {
                        var valArray = [], index;
                        var position = helpers.getRelativePosition(evt);
                        activePoints.forEach(function (val) {
                            valArray.push(Math.abs(position.y - val.y))
                        });
                        var index = indexOfMin(valArray);
                        activePoints = index == null ? [] : [activePoints[index]];
                    }
